...
class MandateDetails(models.Model):
                    ...
                    if resp_obj.get('Messages') and resp_obj.get('Messages')[0] and resp_obj.get('Messages')[0] != []:
                        for data in resp_obj.get('Messages'):
                            if data.get('AmdmntRsn'):
                                ...
                                if not partner_id:
                                    partner_id = self.env['res.partner'].create(
                                        {'name': data.get('Mndt').get('Dbtr').get('Nm')})
                                creditor_id = self.env['res.partner'].search(
                                    [('name', '=', data.get('Mndt').get('Cdtr').get('Nm'))])
                                if not creditor_id:
                                    creditor_id = self.env['res.partner'].create(
                                        {'name': data.get('Mndt').get('Cdtr').get('Nm')})

                                if mandate_id:
                                    mandate_id.write({'reference': data.get('Mndt').get('MndtId') if data.get(
                                        'Mndt').get('MndtId') else False,
                                                      'partner_id': partner_id.id if partner_id else False,
                                                      'state': 'signed',
                                                      'creditor_id': creditor_id.id if creditor_id else False,
                                                      'iban': data.get('DbtrAcct') if data.get('DbtrAcct') else False,
                                                      'bic': data.get('DbtrAgt').get('FinInstnId').get(
                                                          'BICFI') if data.get('DbtrAgt') and data.get('DbtrAgt').get(
                                                          'FinInstnId') and data.get('DbtrAgt').get('FinInstnId').get(
                                                          'BICFI') else False
                                                      })
                                    if partner_id:
                                        address = False
                                        ...
                                        partner_id.write({'street': address,
                                                          ...
                                                          'email': data.get('Mndt').get('Dbtr').get('CtctDtls').get(
                                                              'EmailAdr') if data.get('Mndt').get('Dbtr') and data.get(
                                                              'Mndt').get('Dbtr').get('CtctDtls') and data.get(
                                                              'Mndt').get('Dbtr').get('CtctDtls').get(
                                                              'EmailAdr') else False})
                                    ...
                                    if creditor_id:
                                        ...
                                        creditor_id.write({'street': address,
                                                           ...
                                                           'city': city,
                                                           'country_id': country_id.id if country_id else False,
                                                           'email': data.get('Mndt').get('Cdtr').get('CtctDtls').get(
                                                               'EmailAdr') if data.get('Mndt').get('Cdtr') and data.get(
                                                               'Mndt').get('Cdtr').get('CtctDtls') and data.get(
                                                               'Mndt').get('Cdtr').get('CtctDtls').get(
                                                               'EmailAdr') else False})
                                else:
                            ...
                            if data.get('CxlRsn'):
                                ...
                                if mandate_id:
                                    mandate_id.write(
                                        {'state': 'cancelled', 'description': data.get('CxlRsn').get('Rsn')})
                                else:
                                    mandate_id = self.env['mandate.details'].sudo().create({
                                        'reference': data.get('OrgnlMndtId'),
                                        'state': 'cancelled'
                            ...
                            if data.get('Mndt'):
                                ...
                                if not mandate_id:
                                    mandate_id = self.env['mandate.details'].sudo().create(
                                        {'partner_id': partner_id.id if partner_id else False,
                                         'reference': data.get('Mndt').get('MndtId'),
                                         'creditor_id': creditor_id.id if creditor_id else False, 'state': 'signed'})
                                else:
        ...
        if authorization_token:
            ...
            data = {'mndtId': self.reference if self.reference else '',
                    ...
                    'lastname': customer_name[1] if customer_name and len(
                        customer_name) > 1 and self.partner_id.company_type == 'person' else '',
            ...
            try:
                ...
                if response.status_code == 204:
                    ...
                    context['message'] = "Sync Mandate Successfully."
                    return {
                        'name': 'Success',
                    ...
                    _logger.info('Not Sync Mandate.. %s' % (response.json()))
            except (ValueError, requests.exceptions.ConnectionError, requests.exceptions.MissingSchema,
                    requests.exceptions.Timeout, requests.exceptions.HTTPError) as e:
                raise exceptions.AccessError(
                    _(
    ...
    def write(self, values):
        ...
        if authorization_token:
            ...
            #                     }
            try:
                response = requests.post("https://api.beta.twikey.com/creditor/mandate/update", data=data,
                                         headers={'Authorization': authorization_token})
                if response.status_code == 200:
                    _logger.info('Record Successfully Updated.. %s' % (response.status_code))
