...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        try:
            ...
            twikey_client = (
                ...
                .sudo()
                .get_twikey_client(company=self.provider_id.company_id)
            )
            if twikey_client:
                    ...
                    self.env["twikey.mandate.details"].sudo().create(
                        {
                            ...
                            "country_id": (
                                customer.country_id.id if customer.country_id else False
                            ),
                        }
                    )
                else:
    ...
    def _twikey_prepare_payment_request_payload(
        self, customer, base_url, template, method
    ):
        """
        ...
        payload["redirectUrl"] = (
            urls.url_join(base_url, f"/twikey/status?ref={self.reference}"),
        )
    ...
    def _twikey_prepare_token_request_payload(
        self, customer, base_url, template, method
    ):
    ...
    def _process_notification_data(self, notification_data):
        ...
        if not payment_status:
            _logger.info(
                "No status update for reference %s, setting to pending", self.reference
            )
            self._set_pending()
            return
        ...
        if self.tokenize:
            ...
            if mandate_id.state == "signed":
                ...
                _logger.info(
                    f"Tokenized redirect but mandate ({self.provider_reference}) was {mandate_id.state}"
                )
        ...
        if payment_status == "pending":
            ...
            self._set_canceled(
                "Twikey: " + _("Canceled payment with status: %s", payment_status)
            )
        else:
            _logger.info(
    ...
    def _get_post_processing_values(self):
        ...
        if self.tokenize and values.get("state") in ["draft", "pending"]:
            ...
            if mandate_id and mandate_id.state == "signed":
                ...
                _logger.info(
                    f"Mandate ({self.provider_reference}) was in {mandate_id.state} for ref {self.reference}"
                )
        return values
        ...
        if twikey_client:
            try:
                ...
                self.partner_id.message_post(
                    body=f"Send {self.reference} to Twikey with state={state}"
                )
                self._set_pending(f"Send to Twikey (state={state})")
                # Handle the payment request response.
                _logger.info(
                    "Send transaction with reference %s: %s",
                    self.reference,
