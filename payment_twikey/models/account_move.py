...
class AccountInvoice(models.Model):
    ...
    def _compute_payment_state(self):
        super(AccountInvoice, self)._compute_payment_state()
        for record in self:
            if record.twikey_invoice_state == 'PAID':
                    ...
                    if new_state == "PAID":
                        ...
                        if last_payment:
                            ...
                            if tx:
                                ...
                                refund = tx._create_child_transaction(amount=tx.amount,
                                   provider_reference=id,
                                ...
                                tx.invoice_ids[:1].payment_state = 'not_paid'
                            else:
                ...
                if last_payment:
                    ...
                    if tx:
                        if new_state == "PAID":
                            ...
                            errorcode = "Failed with errorcode={}".format(last_payment["rc"])
                            tx._set_error(errorcode)
                            refund = tx._create_child_transaction(provider_reference=id)
                            refund._set_done(state_message=errorcode)
                            refund._reconcile_after_done()
                            refund._finalize_post_processing()
                    else:
