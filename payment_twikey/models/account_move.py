...
from odoo import SUPERUSER_ID, api, _, fields, models
        ...
        for invoice in self:
            ...
            if invoice.is_purchase_document():
                if invoice.amount_total == 0:
                    ...
                    customer_bank_id = partner_id.bank_ids
                    ...
                    if new_state == "PAID":
                        if last_payment:
                            ...
                            tx = self.get_or_create_payment_transaction({
                                ...
                                'acquirer_reference': id,
                                'operation': "offline",
                                'partner_id': invoice_id.partner_id.id,
                            })
                            tx.invoice_ids = invoice_id.ids
                        ...
                        if last_payment:
                            acquirer_reference = last_payment["e2e"]
                            ...
                            if tx:
                                ...
                                _logger.warning(f"payment.transaction with reference={acquirer_reference} not found")
                                invoice_id.message_post(body=f"payment.transaction with reference={acquirer_reference} not found")
                        else:
